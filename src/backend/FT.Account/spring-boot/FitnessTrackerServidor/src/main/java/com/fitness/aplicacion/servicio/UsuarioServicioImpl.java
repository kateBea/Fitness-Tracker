...
public class UsuarioServicioImpl implements IUsuarioServicio {
    ...
    @Override
    public boolean changePassword(RequestCambiarPassword model) {
        Optional<Usuario> info = _usuarioRepositorio.findById(model.getEmail());
        boolean result = info.isPresent();

        info.ifPresent(user -> {
            boolean match = _passwordEncoder.matches(model.getOldPassword(), info.get().getContrasena());
            boolean oldMatchesNew = match && _passwordEncoder.matches(model.getNewPassword(), info.get().getContrasena());

            if (!match) {
                throw new RuntimeException("Los credenciales son inválidos.");
            }
            ...
            if (!UtilidadesUsuario.passwordCheck(model.getNewPassword())) {
                throw new RuntimeException("La contraseña nueva es débil. Debe tener al menos 8 caracteres, un dígito y uno no alfanumérico.");
            }
